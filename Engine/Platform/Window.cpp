...
bool Window::PollEvents(Input* pInput)
{
	...
	SDL_Event event;
	SDL_WindowID ownId = SDL_GetWindowID(m_pWindow);
	while (SDL_PollEvent(&event)) {
		...
		// Only process window-specific events for our own window.
		// Other windows (e.g. runtime preview) must not interfere.
		bool bIsOurWindow = true;
		if (event.type >= SDL_EVENT_WINDOW_FIRST && event.type <= SDL_EVENT_WINDOW_LAST)
			bIsOurWindow = (event.window.windowID == ownId);
		...
		switch (event.type) {
			...
			case SDL_EVENT_KEY_DOWN:
				if (bIsOurWindow && event.key.scancode == SDL_SCANCODE_ESCAPE)
					return false;
				break;
